  ...
  <q-dialog ref="dialog" @hide="onDialogHide">
    <q-card style="width: 500px" class="bg-white">
      <q-card-section class="text-h6 text-primary q-pb-none">
        ...
        {{ this.mode == 'edit' ? 'Editar ' : 'Criar ' }} Dispositivo de Armazenamento
      </q-card-section>
      <q-card-section class="text-grey-8 text-h6 q-mb-sm">
        <q-form @submit="handleSubmit" class="row q-col-gutter-md">
          ...
          <div class="col-12">
            <q-input
              v-model="obj.connection_technology"
              label="Tecnologia *"
              :rules="[required]"
              hide-bottom-space
              outlined
...
export default {
  name: 'StorageDeviceDialog',
  props: ['storageDevice', 'computer_id'],
  components: {
    ComputerInput
  },
  data: () => ({
  ...
  methods: {
    ...
    async handleSubmit () {
      if (this.mode == 'edit') {
        const { data } = await this.$axios.put('storage-device/' + this.obj.id, { ...this.obj })
        this.$q.notify({
          message: data.message,
          type: 'positive'
        })
      } else {
        const { data } = await this.$axios.post('storage-device', { ...this.obj })
        this.$q.notify({
          message: data.message,
          type: 'positive'
        })
