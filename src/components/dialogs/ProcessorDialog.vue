  ...
  <q-dialog ref="dialog" @hide="onDialogHide">
    <q-card style="width: 500px" class="bg-white">
      <q-card-section class="text-h6 text-primary q-pb-none">
        ...
        {{ this.mode == 'edit' ? 'Editar ' : 'Criar ' }} Processador
      </q-card-section>
...
export default {
  ...
  methods: {
    ...
    async handleSubmit () {
      if (this.mode == 'edit') {
        const { data } = await this.$axios.put('processor/' + this.obj.id, { ...this.obj })
        this.$q.notify({
          message: data.message,
          type: 'positive'
        })
      } else {
        const { data } = await this.$axios.post('processor', { ...this.obj })
        this.$q.notify({
          message: data.message,
          type: 'positive'
        })
