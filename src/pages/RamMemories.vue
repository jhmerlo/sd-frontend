<template>
  <q-page style="min-height: 0px !important">
        ...
        >
          <template #name-actions="{ props }">
            <div class="col-12 q-gutter-xs row no-wrap justify-center">
              <q-item
                @click="showRamMemoryDialog(props.row)"
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 18]">
      <q-btn @click="showRamMemoryDialog(null)" fab icon="add" color="primary">
      </q-btn>
    </q-page-sticky>
  </q-page>
</template>
...
export default {
  ...
  data: () => ({
    ...
    columns: [
      ...
      { name: 'technology', label: 'Tecnologia', field: 'technology', align: 'left', format: v => v ? v : '-'},
  ...
  methods: {
    async getIndex (props) {
      ...
      const { data } = await this.$axios.get('ram-memories', { 
        params: { 
          page,
          ...this.filters
        }
    ...
    showCommentsDialog (commentable) {
      this.$q.dialog({
        ...
        commentableType: "App\\Models\\RamMemory",
        commentable
      }).onOk((comments) => {
