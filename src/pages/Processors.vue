<template>
  <q-page style="min-height: 0px !important">
        ...
        >
          <template #name-actions="{ props }">
            <div class="col-12 q-gutter-xs row no-wrap justify-center">
              <q-item
                @click="showProcessorDialog(props.row)"
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 18]">
      <q-btn @click="showProcessorDialog(null)" fab icon="add" color="primary">
      </q-btn>
    </q-page-sticky>
  </q-page>
</template>
...
export default {
  ...
  data: () => ({
    ...
    columns: [
      ...
      { name: 'clock', label: 'Clock', field: 'clock', align: 'left', format: v => v ? v + ' GHz' : '-'},
      { name: 'threads', label: 'Threads', field: 'threads', align: 'left', format: v => v ? v : '-'},
      { name: 'cache', label: 'Cache', field: 'cache', align: 'left', format: v => v ? v + ' MB' : '-'},
  ...
  methods: {
    async getIndex (props) {
      ...
      const { data } = await this.$axios.get('processors', { 
        params: { 
          page,
          ...this.filters
        }
    ...
    showCommentsDialog (commentable) {
      this.$q.dialog({
        ...
        commentableType: "App\\Models\\Processor",
        commentable
      }).onOk((comments) => {
