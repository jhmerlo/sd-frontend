<template>
  <q-page style="min-height: 0px !important">
            ...
            <q-card-section class="q-pt-sm">
              <q-form @submit.prevent="getIndex">
                <div class="row q-col-gutter-md">
                  ...
                  <div class="col-md-3 col-sm-6 col-xs-12">
                    <q-input
                      label="Tecnologia de Conexão"
                      v-model="filters.connection_technology" 
                      outlined
                      dense
                    />
                  </div>
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 18]">
      <q-btn @click="showStorageDeviceDialog(null)" fab icon="add" color="primary">
      </q-btn>
    </q-page-sticky>
  </q-page>
</template>
...
export default {
  ...
  data: () => ({
    ...
    columns: [
      ...
      { name: 'type', label: 'Tipo', field: 'type', align: 'left', format: v => v ? v : '-'},
  ...
  methods: {
    async getIndex (props) {
      ...
      const { data } = await this.$axios.get('storage-devices', { 
        params: { 
          page,
          ...this.filters
        }
      ...
      this.storage_devices = data.data
    ...
    showStorageDeviceDialog (storageDevice) {
      this.$q.dialog({
    ...
    showCommentsDialog (commentable) {
      this.$q.dialog({
        ...
        commentableType: "App\\Models\\StorageDevice",
        commentable
      }).onOk((comments) => {
