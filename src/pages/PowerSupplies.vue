<template>
  <q-page style="min-height: 0px !important">
        ...
        >
          <template #name-actions="{ props }">
            <div class="col-12 q-gutter-xs row no-wrap justify-center">
              <q-item
                @click="showPowerSupplyDialog(props.row)"
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 18]">
      <q-btn @click="showPowerSupplyDialog(null)" fab icon="add" color="primary">
      </q-btn>
    </q-page-sticky>
  </q-page>
</template>
...
export default {
  ...
  data: () => ({
    ...
    columns: [
      ...
      { name: 'electric_power', label: 'Potência', field: 'electric_power', align: 'left', format: v => v ? v + ' W' : '-'},
      { name: 'voltage', label: 'Tensão', field: 'voltage', align: 'left', format: v => v ? v + ' V' : '-'},
  ...
  methods: {
    async getIndex (props) {
      ...
      const { data } = await this.$axios.get('power-supplies', { 
        params: { 
          page,
          ...this.filters
        }
    ...
    showPowerSupplyDialog (powerSupply) {
      this.$q.dialog({
    ...
    showCommentsDialog (commentable) {
      this.$q.dialog({
        ...
        commentableType: "App\\Models\\PowerSupply",
        commentable
      }).onOk((comments) => {
